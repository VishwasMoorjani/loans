<!DOCTYPE html>
<html>

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <title>Almost near | Log in</title>
  <!-- Tell the browser to be responsive to screen width -->
  <META NAME="ROBOTS" CONTENT="NOINDEX, NOFOLLOW">
  <!-- Bootstrap Css -->
  <link rel="stylesheet" href="<?php echo $this->config->item('admin_assets'); ?>css/bootstrap.min.css" type="text/css" />
  <!-- icons css -->
  <link rel="stylesheet" href="<?php echo $this->config->item('admin_assets'); ?>css/all.css" type="text/css" />
  <link rel="stylesheet" href="<?php echo $this->config->item('admin_assets'); ?>font/flaticon.css" type="text/css" />
  <link rel="stylesheet" href="https://cdn.datatables.net/v/bs4/dt-1.10.20/datatables.min.css" type="text/css" />
  <!-- font -->
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@200;300;400;500;600;700&display=swap" rel="stylesheet" />
  <!-- Custom Css -->
  <link rel="stylesheet" href="<?php echo $this->config->item('admin_assets'); ?>css/style.css" type="text/css" />
  <link href="<?php echo $this->config->item('admin_assets'); ?>css/jquery.toast.min.css" rel="stylesheet">
  <!-- Google Font: Source Sans Pro -->
  <link href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700" rel="stylesheet">
  <link rel="stylesheet" href="<?php echo $this->config->item('admin_assets'); ?>css/responsive.css" type="text/css" />
  <style type="text/css">
    .login_container .login_form {
      width: 800px;
      margin: auto;
      text-align: center;
    }
  </style>
</head>

<body>

  <section id="registration_form_section">
    <div class="container">

      <!-- <div class="form_logo">
  <a href="#">
  <img src="<?php echo $this->config->item('admin_assets'); ?>images/logo.png" class="img-fluid">
  </a>
</div> -->

      <div class="registration_form_container">

        <div class="row">
          <div class="col-md-5">
            <div class="left_image">
              <a href="#">
                <img src="<?php echo $this->config->item('admin_assets'); ?>images/logo.png" class="img-fluid">
              </a>
            </div>
          </div>
          <div class="col-md-7">
            <div class="form_container">
              <h3>Create Account</h3>
              <form class="ajax_form" action="<?php echo site_url('saller/register'); ?>" method="post">
                <div class="form-group row">

                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="fas fa-user"></i>
                      <input type="text" class="form-control" placeholder="Name" name="full_name">
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="fas fa-user"></i>
                      <input type="text" class="form-control" placeholder="User Name" name="username">
                    </div>
                  </div>
                </div>
                <div class="form-group row">

                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="far fa-envelope"></i>
                      <input type="text" class="form-control" placeholder="Email" name="email">
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="fas fa-mobile-alt"></i>
                      <input type="text" class="form-control" placeholder="Mobile" name="mobile_number">
                    </div>
                  </div>
                </div>
                <div class="form-group row">

                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="fas fa-flag"></i>
                      <input type="text" class="form-control" placeholder="Country" name="country">
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="fab fa-usps"></i>
                      <input type="text" class="form-control" placeholder="state" name="state">
                    </div>
                  </div>
                </div>
                <div class="form-group row">

                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="fas fa-city"></i>
                      <input type="text" class="form-control" placeholder="city" name="City">
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="input_container">
                      <i class="fas fa-map-pin"></i>
                      <input type="text" class="form-control" placeholder="Pincode" name="postal">
                    </div>
                  </div>
                </div>


                <div class="form-group row">
                  <div class="col-12">
                    <div class="input_container">
                      <i class="fas fa-file-signature"></i>
                      <input type="text" class="form-control" placeholder="Shope name" name="shope_name">
                    </div>
                  </div>
                </div>
                <div class="form-group row">
                  <div class="col-12">
                    <div class="input_container">
                      <i class="fas fa-file-signature"></i>
                      <input type="text" class="form-control" placeholder="Address" name="address">
                    </div>
                  </div>
                </div>
                <?php if ($admin == 1) { ?>
                  <input type="hidden" name="admin" value="1">
                <?php } ?>
                <div class="form-group row">
                  <div class="col-12">
                    <div class="input_container">
                      <i class="fas fa-file"></i>
                      <input type="file" name="businesp" class="form-control" />
                      <span style="color:red ">Should be adhar card , GST Number,Other Business Doc</span>
                    </div>
                  </div>
                </div>


                <div class="form-group row">
                  <div class="col-12">
                    <div class="input_container">
                      <i class="fas fa-lock"></i>
                      <input type="password" class="form-control" placeholder="Password" name="password">
                    </div>
                  </div>
                </div>


                <div class="form-group row">
                  <div class="col-12">
                    <div class="input_container">
                      <i class="fas fa-lock"></i>
                      <input type="password" class="form-control" placeholder="Confirm password" name="confirm_password">
                    </div>
                  </div>
                </div>


                <div class="form-group row">
                  <div class="col-12">
                    <div class="custom-control custom-checkbox ">
                      <!-- <input type="checkbox" class="custom-control-input" id="customCheck1"> -->
                      <!-- <label class="custom-control-label" for="customCheck1">Remember Me</label> -->
                    </div>
                  </div>
                </div>


                <button class="login_btn" type="submit">Register</button>
              </form>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>




  <!-- <div class="login_bg">
      <div class="login_container d-flex align-items-center justify-content-center">
        <div class="login_form">
          <a href="#" class="login_logo">
            <img src="<?php echo $this->config->item('admin_assets'); ?>images/logo.png" class="img-fluid">
          </a>
          <div class="form_container">
            <h3>Create Your Account</h3>
           <form class="ajax_form" action="<?php echo site_url('saller/register'); ?>" method="post">
            <div class="form-group row">
              
              <div class="col-sm-6">
                <div class="input_container">
                  <i class="fas fa-user"></i>
                  <input type="text" class="form-control" placeholder="Name" name="full_name">
                </div>
              </div>
              <div class="col-sm-6">
                <div class="input_container">
                  <i class="fas fa-user"></i>
                  <input type="text" class="form-control" placeholder="User Name" name="username">
                </div>
              </div>
            </div>
              <div class="form-group row">
              
              <div class="col-sm-6">
                <div class="input_container">
                  <i class="far fa-envelope"></i>
                  <input type="text" class="form-control" placeholder="Email" name="email">
                </div>
              </div>
              <div class="col-sm-6">
                <div class="input_container">
                  <i class="fas fa-mobile-alt"></i>
                     <input type="text" class="form-control" placeholder="Mobile" name="mobile_number">
                </div>
              </div>
            </div>
              <div class="form-group row">
              
              <div class="col-sm-6">
                <div class="input_container">
                 <i class="fas fa-flag"></i>
                   <input type="text" class="form-control" placeholder="Country" name="country">
                </div>
              </div>
              <div class="col-sm-6">
                <div class="input_container">
                  <i class="fab fa-usps"></i>
                     <input type="text" class="form-control" placeholder="state" name="state">
                </div>
              </div>
            </div>
            <div class="form-group row">
              
              <div class="col-sm-6">
                <div class="input_container">
                 <i class="fas fa-city"></i>
                   <input type="text" class="form-control" placeholder="city" name="City">
                </div>
              </div>
              <div class="col-sm-6">
                <div class="input_container">
                  <i class="fas fa-map-pin"></i>
                     <input type="text" class="form-control" placeholder="Pincode" name="postal">
                </div>
              </div>
            </div>
              <div class="input_container">
                <i class="fas fa-file-signature"></i>
                <input type="text" class="form-control" placeholder="Shope name" name="shope_name">
              </div>
                <div class="input_container">
                <i class="fas fa-file"></i>
                <input type="file" name="businesp" class="form-control" />
                <span style="color:red ">Should be adhar card , GST Number,Other Business Doc</span>
              </div>
              <div class="input_container">
                <i class="fas fa-lock"></i>
                <input type="password" class="form-control" placeholder="Password" name="password">
              </div>            
                <div class="input_container">
                <i class="fas fa-lock"></i>
                <input type="password" class="form-control" placeholder="Confirm password" name="confirm_password">
              </div>
              <div class="custom-control custom-checkbox ">
                <!-- <input type="checkbox" class="custom-control-input" id="customCheck1"> -->
  <!-- <label class="custom-control-label" for="customCheck1">Remember Me</label> -->
  <!--</div>
              <button class="login_btn" type="submit">Register</button>
            </form>
          </div>
          <!-- <a href="#">Forgot your Password?</a> -->
  <!-- </div>
      </div>
    </div> -->
  <script src="<?php echo $this->config->item('admin_assets') ?>js/jquery.min.js" type="text/javascript"></script>

  <!-- Bootstrap Js -->
  <script src="<?php echo $this->config->item('admin_assets') ?>js/popper.min.js" type="text/javascript"></script>
  <script src="<?php echo $this->config->item('admin_assets') ?>js/bootstrap.min.js" type="text/javascript"></script>
  <script src="<?php echo $this->config->item('admin_assets') ?>js/jquery.toast.min.js"></script>
  <script src="<?php echo $this->config->item('admin_assets') ?>js/jquery.form.js"></script>
  <script src="<?php echo $this->config->item('admin_assets') ?>js/corp.js"></script>
</body>

</html>