...
class Speical_offer_model extends CI_Model
{

    public function __construct()
    {
        parent::__construct();
        $this->table = 'speical_offter';
    }
    ...
    public function countAllRecords($searchData)
    {
        ...
        $this->db->select('speical_offter.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('speical_offter.title', $searchData['keyword']);
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('speical_offter.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('speical_offter.' . $searchData['column_name'], $searchData['sorting_order']);
