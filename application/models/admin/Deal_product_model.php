...
class Deal_product_model extends CI_Model
{

    public function __construct()
    {
        parent::__construct();
        $this->table = 'deal_product';
    }
    ...
    public function countAllRecords($searchData)
    {
        ...
        $this->db->select('deal_product.id');
        ...
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('deal_product.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('deal_product.' . $searchData['column_name'], $searchData['sorting_order']);

        }
        $this->db->join('products', 'products.product_id = deal_product.product_id');
        $query  = $this->db->get($this->table);
    ...
    public function getAllRecords($searchData)
    {
        ...
        $this->db->select('deal_product.*');
        $this->db->select('products.product_name,products.product_image');
        if (isset($searchData) && $searchData['keyword']) {
