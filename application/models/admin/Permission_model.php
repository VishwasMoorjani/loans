...
class Permission_model extends CI_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->where('tbl_permissions.id', $id);
        $query  = $this->db->get($this->table);
        $result = $query->row();
        return $result;
    }
    ...
    public function countAllRecords($searchData)
    {
        ...
        $this->db->select(' tbl_permissions.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('tbl_permissions.title', $searchData['keyword']);
            //$this->db->having("FullName LIKE '%".$searchData['keyword']."%'");
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('tbl_permissions.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('tbl_permissions.' . $searchData['column_name'], $searchData['sorting_order']);
