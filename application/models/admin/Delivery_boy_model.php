...
class Delivery_boy_model extends CI_Model
{
    ...
    public function countAllRecords($searchData)
    {
        ...
        $this->db->select('delivery_boy.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('delivery_boy.user_name', $searchData['keyword']);
            $this->db->or_like('delivery_boy.user_email', $searchData['keyword']);
            $this->db->or_like('delivery_boy.user_phone', $searchData['keyword']);
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('delivery_boy.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('delivery_boy.' . $searchData['column_name'], $searchData['sorting_order']);
