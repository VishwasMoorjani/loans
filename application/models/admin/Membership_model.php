...
class Membership_model extends CI_Model
{
    ...
    public function countAllRecords($searchData,$store_id)
    {
        ...
        $this->db->select('tbl_membership_plan.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('tbl_membership_plan.title', $searchData['keyword']);
            //$this->db->having("FullName LIKE '%".$searchData['keyword']."%'");
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('tbl_membership_plan.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('tbl_membership_plan.' . $searchData['column_name'], $searchData['sorting_order']);

        }
        if($store_id!=1)
        $this->db->where('tbl_membership_plan.store_id', $store_id);
        $query  = $this->db->get($this->table);
