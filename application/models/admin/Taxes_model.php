...
class Taxes_model extends CI_Model
{
    ...
    public function countAllRecords($searchData)
    {
        ...
        $this->db->select('tbl_taxes.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('tbl_taxes.title', $searchData['keyword']);
            //$this->db->having("FullName LIKE '%".$searchData['keyword']."%'");
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('tbl_taxes.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('tbl_taxes.' . $searchData['column_name'], $searchData['sorting_order']);
