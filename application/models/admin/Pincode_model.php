...
class Pincode_model extends CI_Model
{
    ...
    public function countAllRecords($searchData)
    {
        ...
        $this->db->select('tbl_pincodes.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('tbl_pincodes.pincode', $searchData['keyword']);
            //$this->db->having("FullName LIKE '%".$searchData['keyword']."%'");
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('tbl_pincodes.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('tbl_pincodes.' . $searchData['column_name'], $searchData['sorting_order']);
