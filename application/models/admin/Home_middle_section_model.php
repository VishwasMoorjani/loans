...
class Home_middle_section_model extends CI_Model
{

    public function __construct()
    {
        parent::__construct();
        $this->table = 'tbl_home_middle_section';
    }
    ...
    public function countAllRecords($searchData)
    {
        ...
        $this->db->select('tbl_home_middle_section.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('tbl_home_middle_section.user_name', $searchData['keyword']);
            $this->db->or_like('tbl_home_middle_section.user_email', $searchData['keyword']);
            $this->db->or_like('tbl_home_middle_section.user_phone', $searchData['keyword']);
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('tbl_home_middle_section.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('tbl_home_middle_section.' . $searchData['column_name'], $searchData['sorting_order']);
