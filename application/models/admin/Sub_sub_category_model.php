...
class Sub_sub_category_model extends CI_Model
{
    ...
    public function countAllRecords($searchData,$store_id)
    {
        ...
        $this->db->select('sub_sub_category.id');
        if (isset($searchData) && $searchData['keyword']) {

            $this->db->group_start();
            $this->db->or_like('sub_sub_category.title', $searchData['keyword']);
            //$this->db->having("FullName LIKE '%".$searchData['keyword']."%'");
            $this->db->group_end();
        }
        if (isset($searchData) && $searchData['status']) {
            $this->db->where('sub_sub_category.status', $searchData['status']);
        }
        if (isset($searchData) && $searchData['sorting_order']) {
            $this->db->order_by('sub_sub_category.' . $searchData['column_name'], $searchData['sorting_order']);

        }
        if($store_id!=1)
        $this->db->where('sub_sub_category.store_id', $store_id);
        $query  = $this->db->get($this->table);
